...
export default function StockRefill(): JSX.Element {
  const [mode, setMode] = useState<'kitchen' | 'stock'>('kitchen')
  const [sortBy, setSortBy] = useState<'kitchen' | 'stock'>('kitchen')
  ...
  const handleKitchenChange = useCallback((id: number, v: number) => {
    setKitchenCount((prev) => {
      const next = { ...prev, [id]: v }
      saveLS(LS_KITCHEN, next)
      return next
    })
  }, [])

  const handlePickedChange = useCallback((id: number, v: number) => {
    setPicked((prev) => {
      const next = { ...prev, [id]: v }
      saveLS(LS_PICKED, next)
      return next
    })
  }, [])
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        button { transition: transform 0.1s ease, background 0.15s ease; }
      `}</style>
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* <Segmented
              options={[
                { label: 'Kitchen Order', value: 'kitchen' },
                { label: 'Stock Order', value: 'stock' },
              ]}
              value={sortBy}
              // onChange={(v: 'kitchen' | 'stock') => setSortBy(v)}
              style={{
                background: 'rgba(255,255,255,0.15)',
                width: '100%',
              }}
              styles={{
                item: { color: 'rgba(255,255,255,0.8)', fontWeight: 600, fontSize: 13, textAlign: 'center', flex: 1 },
              }}
            /> */}
          </div>
